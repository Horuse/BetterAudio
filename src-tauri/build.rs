...
fn compile_swift_static_lib() {
    ...
    let swift_src = manifest_dir.join("native").join("SCKAudioCapture.swift");
    if !swift_src.exists() {
        return;
    }
    ...
    if !status.success() {
        panic!("swiftc failed for {}", swift_src.display());
    }
    ...
    for framework in ["ScreenCaptureKit", "CoreMedia", "CoreFoundation", "Foundation"] {
